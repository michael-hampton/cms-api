...
class DealOfferRenderer implements EmailBlockRenderer
{
    ...
    public function render(BaseBlockData $blockData, NewsletterRenderContext $newsletterRenderContext): RenderedBlock
    {
        ...
        try {
            ...
            $eligibility = $this->dealVisibilityResolver->resolve(
                $product,
                $context
            );
            ...
            $this->trackingService->trackRender($product, $newsletterRenderContext);
    ...
    private function renderHtml($product, NewsletterRenderContext $context): string
    {
        ...
        if ($product->main_image_url) {
            ...
            $html[] = '<img src="' . Str::sanitize($product->main_image_url) . '" alt="' . Str::sanitize($product->name) . '" style="width: 150px; height: auto; border-radius: 4px;">';
            $html[] = '</div>';
        }
        ...
        if ($product->brand) {
            $html[] = '<div style="color: #666; font-size: 14px; margin-bottom: 5px;">' . Str::sanitize($product->brand->name) . '</div>';
        }

        $html[] = '<h3 style="color: #333; margin: 0 0 10px 0; font-size: 20px;">' . Str::sanitize($product->name) . '</h3>';

        if ($product->description) {
            $truncated = Str::truncate($product->description, 150);
            $html[] = '<p style="color: #666; margin: 0 0 15px 0; font-size: 14px; line-height: 1.6;">' . Str::sanitize($truncated) . '</p>';
        }
