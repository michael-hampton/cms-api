...
class InfoBlockRenderer implements EmailBlockRenderer
{
    ...
    public function render(BaseBlockData $blockData, NewsletterRenderContext $context): RenderedBlock
    {
        ...
        $baseStyle = "background-color: {$color['bg']}; border-left: 4px solid {$color['border']}; padding: 15px; margin: 20px 0; border-radius: 4px;";
        $wrapperStyle = $blockData->style->mergeIntoCss($baseStyle);

        $baseTextStyle = 'margin: 0; color: #333; font-size: 16px; line-height: 1.6;';
        $textStyle = $blockData->style->mergeIntoCss($baseTextStyle);

        $html = "<div style=\"{$wrapperStyle}\">"
            . "<p style=\"{$textStyle}\"><strong>" . ucfirst($blockData->infoType) . ':</strong> '
            . Str::sanitize($blockData->description)
            . '</p></div>';
