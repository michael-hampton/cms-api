...
class TeaserBlockRenderer implements EmailBlockRenderer
{
    ...
    public function render(BaseBlockData $blockData, NewsletterRenderContext $newsletterRenderContext): RenderedBlock
    {
        ...
        $textColor = in_array($blockData->theme, ['dark', 'primary', 'success']) ? '#ffffff' : '#333333';

        $html = [];
        $html[] = '<div style="background-color: ' . $bgColor . '; color: ' . $textColor . '; padding: 30px 20px; border-radius: 8px; margin: 30px 0;">';

        if ($blockData->copy) {
            $html[] = '<div style="margin-bottom: 25px; font-size: 16px; line-height: 1.6;">' . $blockData->copy . '</div>';
        }
        ...
        if (!empty($blockData->items)) {
            ...
            foreach (array_slice($blockData->items, 0, 3) as $item) {
                ...
                $html[] = '<div style="font-size: 32px; margin-bottom: 10px; color: ' . $textColor . ';">' . $icon . '</div>';
                ...
                if (!empty($item['link'])) {
                    $buttonBg = in_array($blockData->theme, ['dark', 'primary']) ? '#ffffff' : '#007bff';
                    $buttonText = in_array($blockData->theme, ['dark', 'primary']) ? '#333333' : '#ffffff';

                    $html[] = '<a href="' . Str::sanitize($item['link']) . '" style="display: inline-block; padding: 8px 16px; background-color: ' . $buttonBg . '; color: ' . $buttonText . '; text-decoration: none; border-radius: 4px; font-size: 14px; font-weight: bold;">Learn More</a>';
                }
