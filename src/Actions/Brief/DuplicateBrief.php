<?php

namespace App\Actions\Brief;

use App\Models\Brief;
use App\Models\BriefAttachment;

class DuplicateBrief
{
    public static function execute(int $briefId, int $userId): Brief
    {
        $original = Brief::with(['attachments', 'collaborators'])->find($briefId);

        $newBrief = Brief::create([
            'site_id' => $original->site_id,
            'title' => $original->title . ' (Copy)',
            'description' => $original->description,
            'owner_id' => $userId,
            'category_id' => $original->category_id,
            'status' => 'draft',
            'target_word_count' => $original->target_word_count,
            'seo_keywords' => $original->seo_keywords,
            'target_audience' => $original->target_audience,
            'template_id' => $original->template_id
        ]);

        // Copy attachments
        foreach ($original->attachments as $attachment) {
            BriefAttachment::create([
                'type' => $attachment->type,
                'file_url' => $attachment->file_url,
                'file_name' => $attachment->file_name,
                'image_id' => $attachment->image_id,
                'product_id' => $attachment->product_id,
                'url' => $attachment->url,
                'metadata' => $attachment->metadata,
                'sort_order' => $attachment->sort_order,
                'brief_id' => $newBrief->id
            ]);
        }

        LogBriefActivity::execute($newBrief->id, $userId, 'duplicated',
            "Duplicated from brief #{$briefId}");

        return Brief::with(['attachments', 'owner', 'category'])->find($newBrief->id);
    }
}