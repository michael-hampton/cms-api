    ...
    <div class="billing-grid">
        <section class="card billing-card billing-card--payment-methods" aria-labelledby="payment-methods-title">
            <div class="card__body">
                @include('shared/billing/_payment_methods_panel', ['pmHeadingId' => 'payment-methods-title', 'pmShowHeader' => true])
            </div>
        </section>
...
<script>
    class BillingPage {
        ...
        #state = { addresses: [], loadingAddress: true, savingAddress: false, editingAddressId: null, deletingAddressId: null, defaultingAddressId: null };
        #els = {};

        constructor() { this.#bindElements(); this.#attachListeners(); this.#loadAddresses(); }
        #bindElements() { ['billing-address-body','address-modal','address-form','open-address-modal-btn','close-address-modal-btn','cancel-address-modal-btn','save-address-btn','address-errors','address-modal-title'].forEach(id => { this.#els[id] = document.getElementById(id); }); }
        #attachListeners() {
            ...
            this.#els['address-modal']?.addEventListener('click', e => { if (e.target === this.#els['address-modal']) this.#closeAddressModal(); });
            document.addEventListener('keydown', e => { if (e.key !== 'Escape') return; this.#closeAddressModal(); });
        }
        ...
        #render() { this.#renderAddresses(); this.#renderSaveAddressBtn(); }
