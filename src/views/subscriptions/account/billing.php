...
<style>
    ...
    .pm-card__default-badge { font-size: 9.5px; font-weight: 700; letter-spacing: .08em; text-transform: uppercase; background: var(--gold-light); color: var(--gold); padding: 3px 9px; border-radius: 100px; flex-shrink: 0; border: 1px solid rgba(184, 134, 11, .2); }
    .pm-card__actions { display: flex; gap: 6px; flex-shrink: 0; }
    ...
    .address-card__badge { display: inline-block; font-size: 9px; font-weight: 700; text-transform: uppercase; letter-spacing: .05em; padding: 2px 6px; border-radius: 4px; margin-bottom: 8px; background: var(--border); color: var(--ink-soft); }
    ...
    .stripe-field-wrapper { border: 1.5px solid var(--border); border-radius: var(--radius-sm); padding: 14px; background: var(--white); transition: var(--transition); margin-bottom: 14px; min-height: 45px; }
    .stripe-field-wrapper:focus-within { border-color: var(--ink); }
    .stripe-field-label { font-size: 10.5px; font-weight: 600; text-transform: uppercase; letter-spacing: .09em; color: var(--ink-muted); margin-bottom: 10px; }
...
<script>
    class BillingPage {
        ...
        #closeRemoveCard() { this.#els['remove-card-modal']?.classList.remove('open'); this.#setState({ pendingRemoveId: null, submittingRemove: false }); this.#renderRemoveCardBtn(); }
