...
class CreateMenuItemRequest extends FormRequest
{
    ...
    public function after(): array
    {
        return [
            function ($request) {
                ...
                if (in_array($request->get('target_type'), ['page', 'category']) && empty($request->get('target_id'))) {
                    $request->validator->errorCollection()->merge(['target_id' => 'Target ID is required for ' . $request->get('target_type') . ' type.']);
                }

                // Validate custom_url is required for custom/external types
                if (in_array($request->get('target_type'), ['custom', 'external']) && empty($this->custom_url)) {
                    $request->validator->errorCollection()->merge(['custom_url' => 'Custom URL is required for ' . $request->get('target_type') . ' type.']);;
                }
                ...
                if ($request->get('parent_id') && $request->get('menu_id')) {
                    $parent = \App\Models\MenuItem::find($request->get('parent_id'));
                    if ($parent && $parent->menu_id !== $request->get('menu_id')) {
    ...
    protected function prepareForValidation(): void
    {
        ...
        if (!$this->has('open_in_new_tab')) {
            $this->data['open_in_new_tab'] = false;
        }
    }
}