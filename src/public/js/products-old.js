(function() {
    ...
    document.addEventListener('category:select', (event) => {
        const selectedId = event.detail.categoryId; // yields number or null

        if (selectedId) {
            // Update your internal filter array with the selected ID
            state.filters.categoryIds = [selectedId];
        } else {
            // Clear the array if the category is deselected
            state.filters.categoryIds = [];
        }

        // Always reset pagination back to page 1 on filter changes
        state.currentPage = 1;

        // 🚀 TRIGGER YOUR INTERNAL FETCH METHOD
        // Look further down in your IIFE for the function that fetches or
        // renders your products (common names: fetchProducts, loadProducts, applyFilters)
    ...
    function renderTopReview(review) {
        ...
        return `
        <button class="btn-show-review" onclick="event.stopPropagation(); toggleReview(this)">            <span>Top Review</span>
    ...
    data-share='${escapeHtml(JSON.stringify({
            id: product.id,
            name: product.name,
            slug: product.slug,
            price: product.price,
                    ...
                    ${renderMerchants(product.availableMerchants || [])}
                        <div class="product-price">
                            ${product.sale_price && product.sale_price < product.price ? `
                                ...
                                <span class="price-original">${CURRENCY_SYMBOL}${formatPrice(product.price)}</span>
                            ` : `
                                 <span class="price-current">${CURRENCY_SYMBOL}${formatPrice(product.price)}</span>
                            `}
                        </div>
                ...
                <div class="variants-grid">
                    ${variants.map(variant => `
                        <div class="variant-option ${variant.in_stock ? '' : 'disabled'}" 
                            ...
                            ` : ''}
                            <div style="font-size: 0.75rem; color: #64748b;">${CURRENCY_SYMBOL}${formatPrice(variant.final_price)}</div>
                        </div>
                    `).join('')}
                </div>
            </div>
                ...
                <div class="comparison-section">
                    ...
                    ${comparison.category_avg_price ? `
                        <div class="comparison-item">
                            ...
                            <span class="comparison-value">${CURRENCY_SYMBOL}${comparison.category_avg_price}</span>
                        </div>
                    ` : ''}
                    ...
                    ${merchants.slice(0, 3).map(merchant => `
                        <div class="comparison-item">
                            ...
                            <span class="comparison-value">
                                ${CURRENCY_SYMBOL}${formatPrice(merchant.sale_price > 0 ? merchant.sale_price : merchant.price)}
