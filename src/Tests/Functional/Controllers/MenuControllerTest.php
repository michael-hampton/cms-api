...
class MenuControllerTest extends FunctionalTestCase
{
    ...
    public function testShowReturnsMenuById()
    {
        ...
        $response = $this->get("/api/menu/{$menu->id}");
        $this->assertEquals(200, $response->getStatusCode());
        $data = json_decode($response->getContent(), true);
    ...
    {
        $response = $this->get('/api/menu/999');
    ...
    {
        Menu::create(['name' => 'Main Menu', 'slug' => 'main-menu', 'is_active' => true]);
        $response = $this->get('/api/menu/slug/main-menu');
        $this->assertEquals(200, $response->getStatusCode());
        $data = json_decode($response->getContent(), true);
    ...
    public function testStoreCreatesNewMenu()
    {
        ...
        $response = $this->post('/api/menu', $menuData);
    ...
    {
        $response = $this->post('/api/menu', ['name' => 'My Custom Menu']);
        $this->assertEquals(201, $response->getStatusCode());
        $data = json_decode($response->getContent(), true);
    ...
    public function testStoreValidatesUniqueSlug()
    {
        ...
        $response = $this->post('/api/menu', ['name' => 'New', 'slug' => 'existing']);
    ...
    public function testUpdateModifiesMenu()
    {
        ...
        $response = $this->put("/api/menu/{$menu->id}", ['name' => 'Updated Menu', 'description' => 'Updated']);
        $this->assertEquals(200, $response->getStatusCode());
        $data = json_decode($response->getContent(), true);
    ...
    public function testHierarchyReturnsMenuStructure()
    {
        ...
        $response = $this->get("/api/menu/{$menu->id}/hierarchy");
        $this->assertEquals(200, $response->getStatusCode());
        $data = json_decode($response->getContent(), true);
        $this->assertTrue($data['success']);
