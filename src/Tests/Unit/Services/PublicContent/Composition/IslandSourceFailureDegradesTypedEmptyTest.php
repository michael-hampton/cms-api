...
final class IslandSourceFailureDegradesTypedEmptyTest extends TestCase
{
    ...
    #[DataProvider('articleSubTypes')]
    public function test_recirculation_source_failure_leaves_typed_empty_without_page_error(string $pageType): void
    {
        $regions = $this->compose($pageType, SourceResult::degraded('synthetic_failure'));

        self::assertIsArray($regions);
        self::assertArrayHasKey('header', $regions);
        self::assertContains('recirculation', $this->types($regions['after-content'] ?? []));
        // Page still composed; recirculation island is present with degraded empty content from the view.
        self::assertNotContains('page-level-error', $this->types($regions['notices'] ?? []));
    }

    #[DataProvider('nonEditorialTypes')]
    public function test_recirculation_is_not_composed_for_landing_or_content(string $pageType): void
    {
        $regions = $this->compose($pageType, SourceResult::ok([(object) ['id' => 1, 'title' => 'x', 'slug' => 'x']]));

        self::assertNotContains('recirculation', $this->types($regions['after-content'] ?? []));
    }

    public static function articleSubTypes(): array
    {
        return [
            'article' => ['article'],
            'review' => ['review'],
    ...
    public static function nonEditorialTypes(): array
    {
        return [
    ...
    private function compose(string $pageType, SourceResult $recirculation): array
    {
        $page = Mockery::mock(Page::class)->makePartial();
        $page->id = 42;
        $page->page_type = $pageType;
        $page->products = new Collection();
        $page->authors = new Collection([(object) ['name' => 'Author', 'slug' => 'author']]);
        $page->title = 'Example';
        $page->slug = 'example';
        $page->is_paid = false;
        ...
        $configSource->shouldReceive('get')
            ->byDefault()
            ->andReturnUsing(static function (int $siteId, string $key, mixed $default = null) {
                if ($key === 'widgets.recirculation.page_types') {
                    return ['article', 'review', 'buying-guide'];
                }
                if (str_starts_with($key, 'widgets.') && str_ends_with($key, '.page_types')) {
                    return $default ?? ['*'];
                }
        ...
        return $composer->compose(new PublicContentContext(
            ...
            viewData: [
                ...
                'newsletterState' => new NewsletterWidgetState(false, false, '/estate/member/login'),
                'socialShare' => null,
                'links' => [
                    'viewer_state' => '/viewer',
                    'comments' => '/comments',
