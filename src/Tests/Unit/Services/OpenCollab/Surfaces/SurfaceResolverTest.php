...
class SurfaceResolverTest extends TestCase
{
    ...
    public function test_admin_manifests_contain_finance_stats_sections(): void
    {
        ...
        $this->assertSame('admin.payouts.stats', $payouts[0]['key']);
        $this->assertSame('admin_payout_stats_grid', $payouts[0]['component']);
        $this->assertSame('/api/guitar-world/open-collab/admin/payouts?per_page=200', $payouts[0]['endpoint']);

        $this->assertSame('admin.disputes.stats', $disputes[0]['key']);
        $this->assertSame('admin_dispute_stats_grid', $disputes[0]['component']);
